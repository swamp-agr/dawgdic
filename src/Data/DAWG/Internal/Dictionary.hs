...
followChar !l !ix d =
  ...
  in unsafePerformIO do
    trace $ concat [
      "-follow l ", pure $ chr $ fromIntegral l, " ix ", show ix, " nextIx ", show nextIx
      , " u ", show u, " nu ", show nu
      ]
    pure $! if DictionaryUnit.label nu /= fromIntegral l then Nothing else Just nextIx
...
followLength !cs !l !ix d =
    let follow' !ix' [] = Just ix'
        follow' !ix' (!i : !is) = unsafePerformIO do
          trace $ concat ["-followLength ix ", show i, " l ", show l]
          pure $! case followChar (cs !! fromIntegral i) ix' d of
            Nothing -> Nothing
...
dump Dictionary{..} = do
  putStrLn "dictionary"
  putStrLn $ concat ["i\tu(", show dictionarySize, ")"]
  
  forM_ [0 .. dictionarySize - 1] \ix -> do
    putStrLn $ concat
      [show ix, "\t", show $ dictionaryUnits UV.! fromIntegral ix]
